...
static void imprimir_quadra_svg_callback(const char *key, const char *value, void *userdata)
{
    ...
    token = strtok(NULL, ";");
    if (token)
        strncpy(cfill, token, 49);

    token = strtok(NULL, ";");
    if (token)
        strncpy(cstrk, token, 49);

    fprintf(svg_file, "   <svg:rect id=\"%s\" x=\"%f\" y=\"%f\" width=\"%f\" height=\"%f\" ",
            key, x, y, w, h);

    if (strlen(cfill) > 0)
    {
        fprintf(svg_file, "fill=\"%s\" stroke=\"%s\" fill-opacity=\"0.8\" ", cfill, (strlen(cstrk) > 0 ? cstrk : "black"));
    }
    else
    {
        fprintf(svg_file, "fill=\"none\" stroke=\"%s\" ", (strlen(cstrk) > 0 ? cstrk : "black")); // Default se vazio
    }

    fprintf(svg_file, "stroke-width=\"%.1fpx\" />\n", sw);
    fprintf(svg_file, "\n   <svg:text x=\"%f\" y=\"%f\" fill=\"%s\" stroke=\"black\" font-size=\"12\">%s</svg:text>\n",
            x + 5.0, y + 9.0, (strlen(cstrk) > 0 ? cstrk : "black"), key);
}

void svg_gerar_quadras(HashFile hf_quadras, FILE *svg_file)
{
    if (!hf_quadras || !svg_file)
        return;
...
void svg_gerar_decoracoes(void *decoracoes_list, FILE *f)
{
    ...
    // Fechar o grupo se ainda não foi fechado
    if (!fechou_grupo)
    {
        fprintf(f, "   </svg:g>\n");
    }
