...
{
    if (strncmp(s, "Face.", 5) == 0 && s[5] != '\0') return s[5];
    return s[0];
}
...
{
    char buf[256];
    if (!hash_search(hf_quadras, cep, buf, sizeof(buf))) return 0;

    char *t = strtok(buf, ";");          /* campo cep */
    t = strtok(NULL, ";"); double qx = t ? atof(t) : 0;
    t = strtok(NULL, ";"); double qy = t ? atof(t) : 0;
    t = strtok(NULL, ";"); double qw = t ? atof(t) : 0;
    t = strtok(NULL, ";"); double qh = t ? atof(t) : 0;

    switch (face) {
        case 'N': *out_x = qx + num; *out_y = qy + qh; break;
        case 'S': *out_x = qx + num; *out_y = qy;      break;
        case 'L': *out_x = qx;       *out_y = qy + num; break;
        case 'O': *out_x = qx + qw;  *out_y = qy + num; break;
        default:  *out_x = qx + qw / 2; *out_y = qy + qh / 2;
    }
    return 1;
}
...
{
    strncpy(ctx->ultimo_cpf_op,   cpf, 49); ctx->ultimo_cpf_op[49]  = '\0';
    strncpy(ctx->ultima_operacao, op,   9); ctx->ultima_operacao[9] = '\0';
}
...
static void pq_callback(const char *key, const char *value, void *userdata)
{
    ...
    char buf[450];
    strncpy(buf, value, 449); buf[449] = '\0';

    strtok(buf, ";"); strtok(NULL, ";"); strtok(NULL, ";"); strtok(NULL, ";");
    char *cep  = strtok(NULL, ";");
    char *face = strtok(NULL, ";");

    if (!cep || strcmp(cep, st->target_cep) != 0) return;

    st->count++;
    if (!face) return;
    char f = parse_face(face);
    if      (f == 'N' || f == 'n') st->count_n++;
    else if (f == 'S' || f == 's') st->count_s++;
    else if (f == 'L' || f == 'l') st->count_l++;
    else if (f == 'O' || f == 'o') st->count_o++;
}
...
static void censo_callback(const char *key, const char *value, void *userdata)
{
    ...
    char buf[256];
    strncpy(buf, value, 255); buf[255] = '\0';
    ...
    if      (s == 'M' || s == 'm') st->total_m++;
    else if (s == 'F' || s == 'f') st->total_f++;

    if (cep && cep[0] != '\0') {
        st->moradores++;
        if      (s == 'M' || s == 'm') st->moradores_m++;
        else if (s == 'F' || s == 'f') st->moradores_f++;
    } else {
        st->sem_teto++;
        if      (s == 'M' || s == 'm') st->sem_teto_m++;
        else if (s == 'F' || s == 'f') st->sem_teto_f++;
    }
}
...
{
    char cep[50];
    if (sscanf(params, "%49s", cep) != 1) return;
...
static void cmd_pq(char *params, CtxQry *ctx)
{
    ...
    strncpy(ctx->ultima_pq, cep, 49); ctx->ultima_pq[49] = '\0';
...
{
    char cpf[50], buf[300];
    if (sscanf(params, "%49s", cpf) != 1) return;
...
static void cmd_nasc(char *params, CtxQry *ctx)
{
    ...
    if (sscanf(params, "%49s %49s %49s %4s %49s", cpf, nome, sobrenome, sexo, nasc) < 5) return;
...
static void cmd_rip(char *params, CtxQry *ctx)
{
    char cpf[50], buf[300];
    if (sscanf(params, "%49s", cpf) != 1) return;
    if (!hash_search(ctx->hf_pessoas, cpf, buf, sizeof(buf))) return;

    char *nome = strtok(buf, ";");
    for (int i = 0; i < 3; i++) strtok(NULL, ";");  /* sobrenome, sexo, nasc */
    char *cep   = strtok(NULL, ";");
    char *face  = strtok(NULL, ";");
...
static void cmd_mud(char *params, CtxQry *ctx)
{
    ...
    if (sscanf(params, "%49s %49s %19s %d %99s",
               cpf, novo_cep, nova_face, &novo_num, novo_compl) < 4) return;
    ...
    /* Quadrado na posição de ORIGEM (endereço anterior) */
    char *old_cep   = strtok(NULL, ";");
    char *old_face  = strtok(NULL, ";");
    char *old_num_s = strtok(NULL, ";");

    if (old_cep && old_face && old_num_s && old_cep[0] != '\0') {
        double ox = 0, oy = 0;
        if (calcular_coordenadas(ctx->hf_quadras, old_cep,
                                  parse_face(old_face), atoi(old_num_s), &ox, &oy)) {
            char *svg = malloc(256);
            snprintf(svg, 256,
...
{
    if      (strcmp(comando, "rq")    == 0) cmd_rq(params, ctx);
    else if (strcmp(comando, "pq")    == 0) cmd_pq(params, ctx);
    else if (strcmp(comando, "censo") == 0) cmd_censo(ctx);
    else if (strcmp(comando, "h?")    == 0) cmd_h(params, ctx);
    else if (strcmp(comando, "nasc")  == 0) cmd_nasc(params, ctx);
    else if (strcmp(comando, "rip")   == 0) cmd_rip(params, ctx);
    else if (strcmp(comando, "mud")   == 0) cmd_mud(params, ctx);
    else if (strcmp(comando, "dspj")  == 0) cmd_dspj(params, ctx);
    else printf("Comando ignorado ou inválido: %s\n", comando);
}
...
{
    if (ctx->ultimo_cpf_op[0] == '\0') return;
    ...
    if      (strncmp(ctx->ultima_operacao, "nasc", 4) == 0)
        { box_color = "#AAAAFF"; text_symbol = "***";   text_color = "#000080"; }
    else if (strncmp(ctx->ultima_operacao, "rip",  3) == 0)
        { box_color = "black";   text_symbol = "R.I.P."; text_color = "white"; }
    else if (strncmp(ctx->ultima_operacao, "dspj", 4) == 0)
        { box_color = "#786721"; text_symbol = "OUT";   text_color = "#F4EED7"; }

    char *box = malloc(512);
    snprintf(box, 512,
             "<svg:rect width=\"100\" height=\"50\" x=\"1460\" y=\"0\""
             " fill=\"%s\" stroke=\"black\" stroke-width=\"1\""
             " fill-opacity=\"1\" rx=\"20\" ry=\"20\" />", box_color);
    decora(ctx, box);
