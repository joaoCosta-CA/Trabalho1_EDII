...
static void pq_callback(const char *key, const char *value, void *userdata) {
    ...
    if (cep && strcmp(cep, state->target_cep) == 0) {
        ...
        if (face) {
            ...
            if (f == 'N' || f == 'n') state->count_n++;
            else if (f == 'S' || f == 's') state->count_s++;
            else if (f == 'L' || f == 'l') state->count_l++;
            else if (f == 'O' || f == 'o') state->count_o++;
        }
    }
}
...
static void censo_callback(const char *key, const char *value, void *userdata) {
    ...
    char *t = strtok(buffer, ";"); // nome
    t = strtok(NULL, ";"); // sobrenome
    char *sexo = strtok(NULL, ";"); // sexo
    
    if (sexo) {
        if (sexo[0] == 'M' || sexo[0] == 'm') state->m++;
        else if (sexo[0] == 'F' || sexo[0] == 'f') state->f++;
    }
}
...
static int calcular_coordenadas(HashFile hf_quadras, const char *cep, char face, int num, double *out_x, double *out_y) {
    char buffer[256];
    if (!hash_search(hf_quadras, cep, buffer, sizeof(buffer))) return 0;
    ...
    if (face == 'N') { *out_x = qx + num; *out_y = qy + qh; }
    else if (face == 'S') { *out_x = qx + num; *out_y = qy; }
    else if (face == 'L') { *out_x = qx; *out_y = qy + num; }
    else if (face == 'O') { *out_x = qx + qw; *out_y = qy + num; }
    else { *out_x = qx + qw/2; *out_y = qy + qh/2; } // Default center
...
static void tratar_comando_qry(char *comando, char *params, CtxQry *ctx) {
    if (strcmp(comando, "rq") == 0) {
        ...
        fprintf(ctx->f_txt, "censo: Total de habitantes: %d (M: %d, F: %d)\n", st.total, st.m, st.f);
        ...
        if (sscanf(params, "%s", cpf) == 1) {
            if (hash_search(ctx->hf_pessoas, cpf, buffer, sizeof(buffer))) {
                char *nome = strtok(buffer, ";");
                for (int i=0; i<3; i++) strtok(NULL, ";"); // pula sobrenome, sexo, nasc
                char *cep = strtok(NULL, ";");
                char *face = strtok(NULL, ";");
